...
def run(unum):
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        with conn:
            ...
            while True:
                hw_msg = conn.recv(3664)
                if not hw_msg:
                    break
                obsspace = conn.recv(3664)
                actspace = conn.recv(3664)
                obsspace = pickle.loads(obsspace)
                actspace = pickle.loads(actspace)
...
def main():
    threads = [None for _ in [2]]

    for i, unum in enumerate([2]):
