...
import numpy as np
import torch
from Deep_Q_Networks.DQN import import Model as DQN
...
# ------------------------ MODEL CONFIG ------------------------------------- #
    ...
    dqn = DQN(env=hfo_env, config=config, static_policy=test)
    currun_rewards = list()
    ...
    if os.path.isfile(model_path) and os.path.isfile(optim_path):
        dqn.load_w(model_path=model_path, optim_path=optim_path)
        logging.info("Model Loaded")

    if not test:
        if os.path.isfile(mem_path):
            dqn.load_replay(mem_path=mem_path)
            dqn.learn_start = 0
    ...
    for episode in itertools.count():
        ...
        while status == hfo.IN_GAME:
            ...
            if done:
                ...
                frame = dqn.stack_frames(state, done)
            ...
            if done:
                ...
                episode_end(total_reward, episode, dqn, state, frame, logging)
            else:
                next_frame = dqn.stack_frames(next_state, done)

            dqn.update(frame, action, reward,
                         next_frame, int(frame_idx / 16))
            frame = next_frame
            state = next_state

            frame_idx += 1
            save_modelmem(episode, test, dqn, model_path, optim_path,
                          frame_idx, config.EXP_REPLAY_SIZE, logging)

# ------------------------ QUIT --------------------------------------------- #
        if status == hfo.SERVER_DOWN:
            if not test:
                dqn.save_w(path_model=model_path, path_optim=optim_path)
                print("Model Saved")
                dqn.save_replay(mem_path=mem_path)
                print("Memory Saved")
