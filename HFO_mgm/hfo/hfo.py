...
"""
NUM_HFO_ACTIONS = 20
DASH, TURN, TACKLE, KICK, KICK_TO, MOVE_TO, DRIBBLE_TO, INTERCEPT, MOVE, SHOOT, PASS, DRIBBLE, CATCH, NOOP, QUIT, REDUCE_ANGLE_TO_GOAL, MARK_PLAYER, DEFEND_GOAL, GO_TO_BALL, REORIENT = list(
...
IN_GAME, GOAL, CAPTURED_BY_DEFENSE, OUT_OF_BOUNDS, OUT_OF_TIME, SERVER_DOWN = list(
    range(NUM_GAME_STATUS_STATES))
...
hfo_lib.waitAnyState.argtypes = [c_void_p]
hfo_lib.waitAnyState.restype = c_bool
hfo_lib.waitToAct.argtypes = [c_void_p]
hfo_lib.waitToAct.restype = c_bool
hfo_lib.processBeforeBegins.argtypes = [c_void_p]
hfo_lib.processBeforeBegins.restype = c_bool
hfo_lib.getAgent.argtypes = [c_void_p]
hfo_lib.getAgent.restype = c_void_p
...
class HFOEnvironment(object):
    ...
    def act(self, action_type, *args):
        ...
        hfo_lib.act(self.obj, action_type,
                    params.ctypes.data_as(POINTER(c_float)))
    ...
    def isIntercept(self):
        """ Returns if the agent can intercept """
        return hfo_lib.isIntercept(self.obj)

    def getTeammateReachCycle(self):
        """ Returns if the agent can intercept """
        return hfo_lib.getTeammateReachCycle(self.obj)

    def getOpponentReachCycle(self):
        """ Returns if the agent can intercept """
        return hfo_lib.getOpponentReachCycle(self.obj)

    def getSelfReachCycle(self):
        """ Returns if the agent can intercept """
        return hfo_lib.getSelfReachCycle(self.obj)

    def waitAnyState(self):
        return hfo_lib.waitAnyState(self.obj)

    def waitToAct(self):
        return hfo_lib.waitToAct(self.obj)
