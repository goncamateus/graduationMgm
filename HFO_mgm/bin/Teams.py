...
class Team(object):
    ...
    def start_npc_proc(self, launchOpts=None):
        ...
        p = subprocess.Popen(player_cmd.split(
...
class Agent2d(Team):
    def __init__(self, name, baseDir, libDir, binaryName, logDir, record,
        ...
        if record:
            options += ' --record'
        offenseOrder = [7, 8, 11, 9, 10, 6, 3, 2, 4, 5]
...
class Helios19(Team):
    def __init__(self, name, baseDir, libDir, binaryName, host='localhost',
        ...
        binaryPath = os.path.join(baseDir, binaryName)
        options = '--player-config %s/player.conf -h %s -t %s '\
            '--strategy-dir %s/data/strategy --formation-dir %s/data/formations '\
            '--setplay-dir %s/data/setplay --strategy-conf %s/data/strategy.conf '\
            '--chain-search-method BestFirstSearch --evaluator-name Default '\
            '--intercept-evaluator-name Default --max-chain-length 3 '\
            '--max-evaluate-size 3000 ' \
            ...
            '--intercept-conf-dir %s/data/intercept_probability/ '\
            '--opponent-data-dir %s/data/opponent_data/ --audio_shift 0 -p %d' % (baseDir, host,
                                                                                  name, baseDir,
                                                                                  baseDir, baseDir,
                                                                                  baseDir, baseDir,
                                                                                  baseDir, baseDir,
