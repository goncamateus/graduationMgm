...
class Trainer(object):
    ...
    def __init__(self, args, server_port=6001, coach_port=6002):
        ...
        self._numOffense = args.offenseAgents + \
            args.offenseNPCs  # Number offensive players
        self._numDefense = args.defenseAgents + \
            args.defenseNPCs  # Number defensive players
        # =============== COUNTERS =============== #
    ...
    def createTeam(self, requested_team_name, play_offense):
        ...
        teams_dir = os.path.join(os.path.dirname(
            os.path.realpath(__file__)), 'teams')
    ...
    def initComm(self):
        ...
        self.checkMsg('(init ok)', retryCount=15)
        # self.send('(eye on)')
        self.send('(ear on)')
    ...
    def printStats(self):
        print('TotalFrames = %i, AvgFramesPerTrial = %.1f, AvgFramesPerGoal = %.1f'
              % (self._numFrames,
                 self._numFrames /
                 float(self._numTrials) if self._numTrials > 0 else float('nan'),
    ...
    def run(self, necProcesses, offense_team_name, defense_team_name):
        ...
        try:
            ...
            sorted_offense_agent_unums = sorted(
                self._offenseOrder[1:self._offenseAgents+1])
            ...
            for player_num in range(1, 12):
                if agent_num < self._offenseAgents and player_num == sorted_offense_agent_unums[agent_num]:
                    ...
                    necProcesses.append(
                        [agent, 'offense_agent_' + str(agent_num)])
                    agent_num += 1
                else:
                    ...
                    if player_num in offense_unums:
                        ...
                        necProcesses.append(
                            [player, 'offense_npc_' + str(player_num)])
                    else:
                        self.disconnectPlayer(
                            player, player_num, on_offense=True)
            ...
            for player_num in range(1, 12):
                if agent_num < self._defenseAgents and player_num == sorted_defense_agent_unums[agent_num]:
                    ...
                    self._agentPopen.append(agent)
                    necProcesses.append(
                        [agent, 'defense_agent_' + str(agent_num)])
                    agent_num += 1
                else:
                    ...
                    if player_num in defense_unums:
                        self._npcPopen.append(player)
                        necProcesses.append(
                            [player, 'defense_npc_' + str(player_num)])
                    else:
                        self.disconnectPlayer(
                            player, player_num, on_offense=False)
