...
FeatureExtractor::FeatureExtractor(int num_teammates,
                                   int num_opponents,
                                   bool playing_offense) : numFeatures(-1),
...
double FeatureExtractor::get_player_congestion(const rcsc::PlayerCont &players,
...

{
  ...
  for (rcsc::PlayerCont::const_iterator o = players.begin(), end = players.end();
       o != end;
       ++o)
  {
    if (o->goalie())
...
std::vector<float> FeatureExtractor::calcOppsDistsToPoint(const rcsc::WorldModel &wm,
                                                          const rcsc::Vector2D &point)
{
...
std::vector<float> FeatureExtractor::calcMatesDistsToPoint(const rcsc::WorldModel &wm,
                                                           const rcsc::Vector2D &point)
{
  std::vector<float> dists;
  const PlayerCont &mates = wm.teammates();
  for (PlayerCont::const_iterator it = mates.begin(); it != mates.end(); ++it)
  {
    ...
    if (valid(mate))
    {
      float dist;
      float th;
      angleDistToPoint(point, mate.pos(), th, dist);
      dists.push_back(dist);
    }
  }
  return dists;
...
{
  float tolerance_x = .1 * pitchHalfLength;
  return unnormalize(normalized_x_pos, -pitchHalfLength - tolerance_x, pitchHalfLength + tolerance_x);
}
