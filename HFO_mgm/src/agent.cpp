...
bool Agent::isIntercept()
{
  ...
  if (!wm.existKickableTeammate() && (self_min <= 3 || (self_min <= mate_min && self_min < opp_min + 3)))
    return true;
  else
    return false;
  return world().self().unum();
  ...
  if (audioSensor().trainerMessageTime().cycle() > lastTrainerMessageTime)
  {
    ...
    if (feature_extractor == NULL)
    {
      ...
      if (hfo::ParseConfig(message, hfo_config))
      {
        ...
        num_teammates = playing_offense ? hfo_config.num_offense - 1 : hfo_config.num_defense - 1;
        num_opponents = playing_offense ? hfo_config.num_defense : hfo_config.num_offense;
...
void Agent::ProcessTeammateMessages()
{
  hear_msg.clear();
...
bool Agent::doPassTo(int receiver)
{
  Force_Pass pass;
  const WorldModel &wm = this->world();
  pass.get_pass_to_player(wm, receiver);
  if (pass.execute(this) || wm.self().collidesWithBall())
  { // can sometimes fix
    return true;
  }
  return false;
...
bool Agent::doMarkPlayer(int unum)
{
  ...
  for (PlayerPtrCont::const_iterator it = wm.opponentsFromSelf().begin(); it != o_end; ++it)
  {
    ...
    {
      if ((kicker_unum == -1) || (kicker_unum != unum))
...
*/
bool Agent::doForceKick()
{
  const WorldModel &wm = this->world();

  if (wm.gameMode().type() == GameMode::PlayOn && !wm.self().goalie() && wm.self().isKickable() && wm.existKickableOpponent())
  {
    dlog.addText(Logger::TEAM,
